...
function DetailedUser(props) {
    ...
    const slug_user = window.location.href.substring(
        window.location.href.lastIndexOf("/") + 1
    );
    console.log(`slug_user`, slug_user)
                ...
                <div className={classes.mainTop} > 
                    <Container>
                        <Box display="flex">
                            ...
                            <Box>
                                <Typography variant='h6' style={{marginLeft: 20, fontWeight: 'bold', fontFamily: 'serif'}}>{detailedUser.fullName}</Typography>
                                <Typography style={{color: "#9eadb6", fontSize: 14, marginLeft: 20}}>{`@${detailedUser.email.replace(/@.*$/,"")}`}</Typography>
                            </Box>
                            <Button variant="outlined" startIcon={<AddIcon/>} color='primary' style={{margin: '8px 0px 0px 20px ', height: 40, }}>Theo dõi</Button>
                        </Box>
                    </Container>
                    <UserTabsBar {...props} onChange={(category) => onCategoryChange(category)} postes={postes} limit={limit} page={page} user={detailedUser} postOfUser={postOfUser} onPaginationChange={onPaginationChange} />
                    <ScrollTop {...props}>
